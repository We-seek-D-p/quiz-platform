...
def create_tokens(user_id: UUID, token_version: int) -> dict[str, str]:
    now = datetime.now()
...
def decode_token(token: str, expected_type: str = "access") -> tuple[UUID | None, int | None]:
    try:
        ...
        if user_id is None:
            return None, None
        try:
            return UUID(user_id), token_version
